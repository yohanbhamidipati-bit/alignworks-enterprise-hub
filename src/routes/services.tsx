...
import { PageHero, SectionHeading, FinalCTA, FeatureGrid } from "@/components/aw-ui";

export const Route = createFileRoute("/services")({
  head: () => ({
    meta: [
      { title: "Services — Technology Consulting for Mission Success | ALIGNWORKS" },
      { name: "description", content: "AI, cybersecurity, compliance, IV&V, testing, and digital transformation services for government and enterprise organizations." },
      { property: "og:title", content: "ALIGNWORKS Services" },
      { property: "og:description", content: "Technology services designed for mission success." },
...
function ServicesPage() {
  return (
    <>
      <PageHero
        eyebrow="Services"
        title="Technology Services Designed for Mission Success"
        description="A unified service architecture covering the disciplines that define successful modernization — strategy, engineering, security, compliance, and independent oversight."
      />

      <section className="section-aw">
        <div className="container-aw">
          <SectionHeading eyebrow="Capabilities" title="Capabilities Overview" />
          <div className="mt-16 grid md:grid-cols-2 lg:grid-cols-3 gap-px bg-border border border-border">
            {CAPS.map((c) => (
              <Link key={c.title} to={c.to} className="bg-background p-10 hover:bg-[var(--surface)] transition-colors block">
                ...
                <span className="mt-6 inline-block text-[13px] font-semibold tracking-[0.14em] uppercase text-foreground border-b border-foreground pb-1">Explore →</span>
              </Link>
            ))}
          </div>
        </div>
      ...
      <section className="section-aw border-t border-border bg-[var(--surface)]">
        <div className="container-aw">
          <SectionHeading eyebrow="Methodology" title="Delivery Methodology" description="Our six-stage delivery framework — Assess, Design, Implement, Validate, Optimize, Govern — is adapted to the regulatory and operational context of every engagement." />
        </div>
      </section>
      ...
      <section className="section-aw">
        <div className="container-aw">
          ...
          <div className="mt-16"><FeatureGrid items={ENGAGEMENT} cols={2} /></div>
        </div>
      </section>
